...
if (!process.env.MC_USERNAME || process.env.MC_USERNAME === "Suva") {
  if (process.env.MC_USERNAME === "Suva") {
    console.warn('[INFO] Using default offline username "Suva". Ensure MC_HOST and MC_PORT are correctly set for your offline server.');
  } else {
...
if (config.minecraft.username === 'Suva' && !config.minecraft.password && !config.minecraft.auth) {
    console.warn('[INFO] Bot is configured for offline mode with default username "Suva". Ensure MC_HOST and MC_PORT are correctly set for your offline server.');
}
