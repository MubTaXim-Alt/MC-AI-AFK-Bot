...
function startBot() {
  ...
  const botScriptPath = path.join(__dirname, 'bot', 'index.js'); // Correct path after tsc compilation
...
const server = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    const frontendDistPath = path.join(__dirname, '..', 'dist'); // Assuming server.js is in dist/
    ...
    fs.readFile(filePath, (error, content) => {
        if (error) {
            if(error.code == 'ENOENT') {
                ...
                fs.readFile(path.join(frontendDistPath, 'index.html'), (err, indexContent) => {
                    if (err) {
                        res.writeHead(500);
